...
%
% This file implements the right hand side of the ODE with dynamic
% external nutrient and population growth with two competitive strains

%%
function dydt= cellmodel_odes_external_2_edit(t, y, rates, parameters)
    ...
    % define location of variables in y array
    ...
    % ODEs from taken directly from paper
	dydt(size(y,1),1)= 0;
    % rate of change.... strain 1
    ...
    dydt(15)= +lam1*n1-dn*n1;               % n1
    
	dydt(16)= +kb*r2*mr2-ku*rmr2-gamma2/nr*rmr2-f*rmr2-lam2*rmr2;   % rmr2
	dydt(17)= +gamma2/nx*rmm2-lam2*em2;                             % em2
	dydt(18)= +kb*r2*mq2-ku*rmq2-gamma2/nx*rmq2-f*rmq2-lam2*rmq2;   % rmq2
	dydt(19)= +kb*r2*mt2-ku*rmt2-gamma2/nx*rmt2-f*rmt2-lam2*rmt2;   % rmt2
	dydt(20)= +gamma2/nx*rmt2-lam2*et2;                             % et2
	dydt(21)= +kb*r2*mm2-ku*rmm2-gamma2/nx*rmm2-f*rmm2-lam2*rmm2;   % rmm2
	dydt(22)= +(we*a2/(thetax + a2))+ku*rmt2+gamma2/nx*rmt2-kb*r2*mt2-dm*mt2-lam2*mt2; % mt2
	dydt(23)= +(we*a2/(thetax + a2))+ku*rmm2+gamma2/nx*rmm2-kb*r2*mm2-dm*mm2-lam2*mm2; % mm2
	dydt(24)= +gamma2/nx*rmq2-lam2*q2;  % q2
	dydt(25)= +nuimp2-nucat2-lam2*si2;  % si2
	dydt(26)= +(wq*a2/(thetax + a2)/(1 + (q2/Kq)^nq))+ku*rmq2+gamma2/nx*rmq2-kb*r2*mq2-dm*mq2-lam2*mq2; % mq2
	dydt(27)= +(wr*a2/(thetar + a2))+ku*rmr2+gamma2/nr*rmr2-kb*r2*mr2-dm*mr2-lam2*mr2;                  % mr2
	dydt(28)= +ku*rmr2+ku*rmt2+ku*rmm2+ku*rmq2+gamma2/nr*rmr2+gamma2/nr*rmr2+gamma2/nx*rmt2+gamma2/nx*rmm2+gamma2/nx*rmq2-kb*r2*mr2-kb*r2*mt2-kb*r2*mm2-kb*r2*mq2-lam2*r2; % r2
	dydt(29)= +ns*nucat2-ttrate2-lam2*a2; % a2
    dydt(30)= +lam2*n2-dn*n2;             % n2
    
   
    dydt(31)= kin-nuimp1*n1-nuimp2*n2-ds*s0;    %s0